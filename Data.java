...
public class Data {
	...
	static private int[][] collisionArray = new int[size][6];
	...
	public static int[][] getArray() {
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader("Crash_Data.csv"));
			String line;
			...
			for (int i = 0; i < getCollisionCount(); i++) {
				...
				if (line.charAt(0) == '2') { // Only consider the rows with
					...
					collisionArray[i][1] = Integer.valueOf(splitline[36]); // XCOORD
					collisionArray[i][2] = Integer.valueOf(splitline[37]); // YCOORD
	...
	public static int getXcrd(int crashNumb) {
		return collisionArray[crashNumb][1];
	}

	public static int getYcrd(int crashNumb) {
		return collisionArray[crashNumb][2];
	}

	public static int getCSEV(int crashNumb) {
		return collisionArray[crashNumb][3];
	}

	public static int getWeather(int crashNumb) {
		return collisionArray[crashNumb][4];
	}

	public static int getPaved(int crashNumb) {
